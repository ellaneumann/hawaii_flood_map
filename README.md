...
- **Tablet** (768–1200px): Sidebar collapses to 280px width, touch-friendly controls
- **Phone** (480–768px): Full-screen collapsible sidebar (overlay mode), optimized text sizing
- **Small Phone** (<480px): All text, buttons, and controls scaled for 320px width

### Responsive Features
- **Collapsible Sidebar**: Click toggle button (top-left) to show/hide layer panel
  - Desktop: Sidebar starts open, stays visible
  - Mobile: Sidebar starts closed, opens as overlay
  - Smooth 0.25s animated transitions
- **Adaptive Typography**: Font sizes adjust by device breakpoint
  - Headers: 16px (desktop) → 14px (tablet) → 13px (phone)
  - Content: 10–11px → 9–10px → 8–9px
- **Touch-Friendly Controls**: Buttons and checkboxes enlarged for mobile (16–18px)
- **Optimized Legend**: Resizes to fit available screen space
- **Flexible Layer List**: Checkbox width and label spacing adjust per device

### Mobile Usage
1. **Toggle Sidebar**: Tap the collapse button (left edge, top-left corner)
2. **Layer Controls**: Tap checkboxes to show/hide map layers  
3. **Map Interaction**: Pinch to zoom, two-finger pan on touchscreen
4. **Popups**: Tap any zone/marker for details (auto-sized to fit screen)
5. **Landscape Mode**: Optimal experience in either orientation on phone

### Testing Responsive Design
- Use browser DevTools: `Ctrl+Shift+M` (Chrome/Firefox) or `Cmd+Shift+M` (Safari)
- Test viewport sizes: 320px, 375px (iPhone SE), 480px, 768px (iPad), 1024px+
- Test touch gestures: Use device emulator or actual mobile device

## Browser Support

- Modern browsers with ES6 support
- Leaflet.js is compatible with all current browsers
...
- Fast initial load: Map renders in <2s
- Smooth interactions: Custom optimizations for tile layer switching
- Lazy news loading: News articles fetched on-demand

## License

This project is a public educational resource documenting the March 2026 Hawaii flood event using official data from government agencies and research institutions.

## Contact & Attribution

**Project**: Hawaii FloodWatch  
**Event**: March 10–24, 2026 Kona Low flooding  
**Last Updated**: April 2026  

Data sourced from:
- FEMA, NWS, USGS, NASA, Hawaii DLNR, University of Hawaii
- Governor Josh Green's Disaster Declaration
