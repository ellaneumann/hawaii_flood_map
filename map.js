...
// ── MAP INITIALIZATION ─────────────────────────────────────────────────────────
var map = L.map('map',{center:[20.5,[-157.5]],zoom:9,zoomControl:false,layers:[SAT]});
map.setView([20.5,-157.5],9);
...
Object.keys(SOIL_DATA).forEach(function(island_key){
  SOIL_DATA[island_key].forEach(function(s){
    ...
    html+='<span class="pop-badge" style="background:'+border_color+'">'+iron_level+' IRON</span>';
    ...
    html+='<div class="pop-box danger" style="font-size:11px"><strong>Flood Hazard:</strong> '+s.flood_impact+'</div>';
    html+='<div class="pop-box info" style="font-size:10px"><strong>Hazards:</strong> '+s.hazards.join(' • ')+'</div>';
    html+='<a href="'+s.source_url+'" target="_blank" style="font-size:9px;color:#0066cc;display:block;margin-top:4px">USDA Soil Series Description</a></div>';
    var fill_color = border_color.split('#')[1];
    L.polygon(s.coords,{fillColor:'rgba('+parseInt(fill_color.substr(0,2),16)+','+parseInt(fill_color.substr(2,2),16)+','+parseInt(fill_color.substr(4,2),16)+',.25)',color:border_color,weight:2,dashArray:'2,3',fillOpacity:.25})
